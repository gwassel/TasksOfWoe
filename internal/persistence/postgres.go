...
func (db *DB) ListTasks(userID int64) ([]domain.Task, error) {
	...
	err := db.Select(&tasks, "SELECT * FROM tasks WHERE user_id = $1 AND completed=FALSE ORDER BY id ASC", userID)
	return tasks, err
}

func (db *DB) ListAllTasks(userID int64) ([]domain.Task, error) {
	var tasks []domain.Task
	err := db.Select(&tasks, "SELECT * FROM tasks WHERE user_id = $1 ORDER BY completed_at NULLS FIRST, id ASC", userID)
	return tasks, err
}

func (db *DB) CompleteTask(userID int64, taskID int64) error {
	_, err := db.Exec("UPDATE tasks SET completed = TRUE, completed_at=NOW() WHERE id = $1 AND user_id = $2 AND completed = FALSE", taskID, userID)
	return err
}
